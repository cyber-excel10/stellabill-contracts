use crate::{
    ...
    Error, OraclePrice, RecoveryReason, Subscription, SubscriptionStatus,
...
fn test_batch_charge() {
    ...
    let (id1, _, _) = fixtures::create_subscription_detailed(&test_env.env, &test_env.client, SubscriptionStatus::Active, AMOUNT, INTERVAL);
...
fn test_multi_token_balances_are_isolated_per_token() {
    ...
    test_env.stellar_token_client().mint(&subscriber_a, &200_000_000i128);
    soroban_sdk::token::StellarAssetClient::new(&test_env.env, &token_b)
        .mint(&subscriber_b, &200_000_000i128);
    ...
    let id_b = test_env.client.create_subscription_with_token(
        &subscriber_b,
        &merchant,
        &token_b,
        &7_000_000i128,
    ...
    test_env.client.deposit_funds(&id_a, &subscriber_a, &200_000_000i128);
    test_env.client.deposit_funds(&id_b, &subscriber_b, &200_000_000i128);

    test_env.env.ledger().set_timestamp(T0 + INTERVAL);
    test_env.client.charge_subscription(&id_a);
    test_env.client.charge_subscription(&id_b);
    ...
    assert_eq!(
        test_env.client.get_merchant_balance_by_token(&merchant, &token_b),
        7_000_000i128
    );
}
